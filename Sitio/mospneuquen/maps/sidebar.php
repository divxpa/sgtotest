...
<div class="subgroup">
	<ul id="organismos">
		...
		<li style="color:<?php echo $row->Color; ?>">
			<input id="organismo_<?php echo $row->IdOrganismo; ?>" type="checkbox" checked="checked" onclick="RefreshData();"><?php echo $row->Nombre; ?>
		</li>
		<?php
			}
...
<div class="titulo">
<input type="checkbox" checked="checked" onclick="CheckUncheckGroup('estado',this.checked);RefreshData();">
<span class="group">ESTADO DE OBRA</span></div>
<div class="subgroup">
	<ul id="estados">
		<?php
			$sql = "select * from estadoobra order by Descripcion";
			$query = mysql_query($sql); 
		...
		<li>
			...
			<input id="estado_<?php echo $row->IdEstadoObra; ?>" type="checkbox" checked="checked" onclick="RefreshData();"><?php echo $row->Descripcion; ?>
		</li>
		<?php
			}
